...
exports.getClub = async (req, res) => {
    if (req.query.id !== undefined) {
        ClubsPage.find({ _id: req.params.id })
            .then((data) => res.status(200).send(data))
            .catch((err) => res.status(400).send("Error: " + err));
    }
    else {
        ClubsPage.find({ show: true })
            .then((data) => res.status(200).send(data))
            .catch((err) => res.status(400).send("Error: " + err));
    }
};
...
exports.getClubbyName = async (req, res) => {
    ...
    ClubsPage.find({ name: name })
        .then((data) => res.status(200).send(data))
        .catch((err) => res.status(400).send("Error: " + err));
};

exports.updateClub = async (req, res) => {
    ClubsPage.findByIdAndUpdate(req.params.id, {
        name: req.body?.name,
        desc: req.body?.desc,
        type: req.body?.type,
        img: req.body?.img,
...
exports.deleteClub = async (req, res) => {
    ClubsPage.findByIdAndUpdate(req.params.id, { $set: { show: false } })
        .then(() => {
